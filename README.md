...
- Points that **never escape** (counted up to `maxiter`) are drawn **black** — they are *inside* the set.
...
|------|-------------|---------------|
| Real (horizontal) | 0 → 639 | −2.5 → 1.0 |
| Imaginary (vertical) | 0 → 639 | −1.5 → 1.5 |

```
a  = xmin + xcoord × Δx       Δx = (xmax − xmin) / 640
bi = ymin + ycoord × Δy       Δy = (ymax − ymin) / 640
```
...
```
R = (i % 5)  × 51         — cycles through 5 red levels
G = 255 − (i % 4) × 64   — cycles through 4 green levels
B = 50 + i / 2            — slowly brightens with depth
```
